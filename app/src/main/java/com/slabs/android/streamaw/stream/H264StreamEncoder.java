...
import com.slabs.android.streamaw.libstream.LibStreamHelper;
...
public class H264StreamEncoder {
    private static final String TAG = H264StreamEncoder.class.getName();
    /*
    height
    width
    sliceHeight
    ...
    private int mWidth=800, mHeight=600, mSize;
    ...
    public static H264StreamEncoder create(SharedPreferences prefs){
        H264StreamEncoder encode = new H264StreamEncoder(prefs);
        encode.printEndingPreferecnes();
        return encode;
    }
    private H264StreamEncoder(SharedPreferences prefs){
        mPreferences =prefs;
        reset();
    }
    ...
    private void printEndingPreferecnes(){
        ...
        LibStreamHelper.Codec[] encoders =LibStreamHelper.findEncodersForMimeType(MIME_TYPE);
        LibStreamHelper.Codec[] decoders = LibStreamHelper.findDecodersForMimeType(MIME_TYPE);
        ...
        for (int i=0;i<encoders.length;i++) {
            for (int j=0;j<encoders[i].formats.length;j++) {
                ...
                LogUtil.d(TAG, ">> Test "+(n++)+"/"+count+": "+mEncoderName+" with color format "+mEncoderColorFormat+" at "+mWidth+"x"+mHeight);
                ...
                try {
                    ...
                    LogUtil.d(TAG, "SPS and PPS in b64: SPS="+mB64SPS+", PPS="+mB64PPS);
                    ...
                    for (int k=0;k<decoders.length && !decoded;k++) {
                        for (int l=0;l<decoders[k].formats.length && !decoded;l++) {
                            ...
                            try {
                               ...
                               LogUtil.d(TAG, mDecoderName+" can't be used with "+mDecoderColorFormat+" at "+mWidth+"x"+mHeight);
                                releaseDecoder();
                                break;
                            }
                            try {
                                decode(true);
                                LogUtil.d(TAG, mDecoderName+" successfully decoded the NALs (color format "+mDecoderColorFormat+")");
                                decoded = true;
                            } catch (Exception e) {
                                LogUtil.e(TAG, mDecoderName+" failed to decode the NALs");
                                e.printStackTrace();
                            } finally {
                                releaseDecoder();
                            }
                    ...
                    if ((padding = checkPaddingNeeded())>0) {
                        if (padding<4096) {
                            LogUtil.d(TAG, "Some padding is needed: "+padding);
                    ...
                    if (!compareChromaPanes(false)) {
                        if (compareChromaPanes(true)) {
                            ...
                            LogUtil.d(TAG, "U and V pane are reversed");
                        } else {
    ...
    private boolean checkTestNeeded() {
        ...
        // Forces the test
        if (mPreferences==null) return true;
        ...
        for (int k=0;k<NB_DECODED;k++) {
            if (mDecodedVideo[k] != null) {
                ...
                if (i>0) {
                    r[k] = ((i>>6)<<6);
                    max = r[k]>max ? r[k] : max;
                    LogUtil.e(TAG,"Padding needed: "+r[k]);
                } else {
                    LogUtil.d(TAG,"No padding needed.");
                }
            }
        }
    ...
    private long encode() {
        ...
        while (elapsed<5000000) {
            ...
            if (encInputIndex>=0) {
                ...
                LogUtil.d(TAG,"No buffer available !");
            }
            ...
            if (decOutputIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                ...
                if (n>2) {
                    ...
                    if (j>=NB_DECODED-1) {
                        ...
                        LogUtil.d(TAG, "Decoding "+n+" frames took "+elapsed/1000+" ms");
                        return elapsed;
                    }
                    j++;
                }
